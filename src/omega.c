...
void Init_cosmomodel_param(double Tinit, double eta, double Nnu, double dNnu, double life_neutron, double xinu1,
                           double xinu2, double xinu3, int plot_abundancesVStime, int plot_NeffVSmassWIMP,
 ...
 *  and the neutron lifetime life_neutron */
{
    ...
    paramrelic->plot_abundancesVStime=plot_abundancesVStime;
    // Plot of Neff vs WIMP mass only made if WIMP run.
    if (paramrelic->wimp_added) paramrelic->plot_NeffVSmassWIMP=plot_NeffVSmassWIMP;
    else paramrelic->plot_NeffVSmassWIMP = 0;
	return;
}
...
void Init_wimp(double m_chi, int SMC_wimp, int beta, int selfConjugate, struct relicparam* paramrelic)
/* modifies the parameters of an included light WIMP */
{
    paramrelic->m_chi=m_chi;
    paramrelic->SMC_wimp=SMC_wimp;
    paramrelic->beta=beta;
    paramrelic->selfConjugate=selfConjugate;
    paramrelic->wimp_added=1;                   // True
    return;
}
