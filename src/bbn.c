    ...
    while(ltime == 0)
    {
        for(loop=1;loop<=2;loop++)
        {
            ...
            else
            {
                ...
                dlna3_dT9=-(drho_gamma_dT9+drho_epem_dT9+drho_wimp_dTvar+drho_epem_dphie*dphie_dT9
                            +rho_baryons*1.388e-4*sum_Y+T9*1e9*dsd_dT9)/
                        (rho_gamma+P_gamma+rho_epem+P_epem+rho_wimp+P_wimp+rho_baryons*(9.25e-5*T9*sum_Y
                                                                                        +1.388e-4*T9*sum_dY_dt/(H*3.)   //E... 2/3*zeta = 9.25e-5 - P_b
                                                                                        +sum_DeltaMdY_dt/(H*3.))
                         +T9*1.e9*sd+drho_epem_dphie*(dphie_dlna3+dphie_dZY*sum_ZdY_dt/(H*3.)));
            }
